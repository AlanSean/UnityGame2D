            ...
            foreach (string guid in list)
            {
                ...
                foreach (Sprite s in sprites)
                {
                    ...
                    spriteDataList.Add(key,new WXSpriteData() {
                        ...
                        colors = new Color[s.texture.height,s.texture.width]

                    });

                    for (var h = 0; h < s.texture.height; h++) {

                        for (var w = 0; w<s.texture.width;w++)
                        {
                            spriteDataList[key].colors[h, w] = s.texture.GetPixel(w, h);
                        }
                    }
                ...
                foreach (Sprite s in sprites) {
                    ...
                    for (int hs = 0; hs< sRect.height; hs++)
                    {
                        ...
                        for (int ws = 0; ws < sRect.width; ws++)
                        {
                            ...
                            if (y >= colors.GetLength(0)) {
                                y--;
                            }
                            var x = ws + (int)System.Math.Round(sRect.x);
                            if (x >= colors.GetLength(1)) {
                                x--;
                            }
